...
PREPARE authorsCreateCommand FROM @authorsStruct;
EXECUTE authorsCreateCommand;
...
SET @booksStruct = CONCAT(
	'CREATE TABLE books (
		...
		ean_id int,
		PRIMARY KEY (id),
		CONSTRAINT books_authors FOREIGN KEY (author_id) REFERENCES ', DATABASE(), '2.authors (id),
		CONSTRAINT books_translator FOREIGN KEY (translator_id) REFERENCES ', DATABASE(), '2.authors (id),
...
PREPARE booksCreateCommand FROM @booksStruct;
EXECUTE booksCreateCommand;
...
SET @tagFollowersStruct = CONCAT(
	'CREATE TABLE tag_followers (
		...
		CONSTRAINT tag_followers_author FOREIGN KEY (author_id) REFERENCES ', DATABASE(), '2.authors (id) ON DELETE CASCADE ON UPDATE CASCADE
	);'
);
PREPARE tagFollowersCommand FROM @tagFollowersStruct;
EXECUTE tagFollowersCommand;
